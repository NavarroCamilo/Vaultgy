...
export class UserController {
  ...
  async addMyReview(
    ...
    const inLibrary = await this.libraryService.isInLibrary(
      request.user!.id,
      gameId,
    );
    ...
    if (!inLibrary) {
      throw new BadRequestException(
        'Game must be in library before creating a review',
      );
    }

    return this.reviewService.createForUser(
      request.user!.id,
      gameId,
      createMyReviewDto,
    );
  ...
  updateMyReview(
    ...
    return this.reviewService.updateByUser(
      request.user!.id,
      reviewId,
      updateReviewDto,
    );
